...
object MiniWindow {
    ...
    /** Whether leaving Vigilant should shrink it to the mini window. */
    fun shouldAutoEnter(settings: ScanSettings, status: ScanStatus, feedSize: Int): Boolean =
        settings.miniWindow && (status.scanning || status.rechecking || feedSize > 0)
    ...
    fun params(context: Context, autoEnter: Boolean, scanning: Boolean): PictureInPictureParams {
