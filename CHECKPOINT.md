...
**Written:** 2026-09-26T02:17:54Z · **tests:** all 1 fast checks green
**Branch:** `claude/positive-ev-betting-research-dkp593` · **builds on:** `e792acb` (this checkpoint is the commit after it)

## Just done
C1-C5 done: RESEARCH.md §16 (CNO real Novig +EV at $5-15 depth; OddsAssist headline edges are longshot noise, ToS bars scraping), BRIEF/CLAUDE docs, build trap 6 corrected (settings-level mirror + Robolectric repo property), version 0.11.0 (code 15). Local: 256 tests green (exit 0, 0 failures), assembleRelease OK (4.78MB).

## Do this next
C6: wait for CI on this commit, then ship.sh, trigger release.yml, confirm Release, record-release, send Tj the link
